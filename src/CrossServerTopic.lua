...
CrossServerTopic.new = Typer.AssignSignature(Typer.String, function(topicName)
...
CrossServerTopic.Send = Typer.AssignSignature(2, Typer.NonNil, function(self, message)
	...
	elseif WARN_WHEN_USING_FUNCTIONS_IN_STUDIO then
		Debug.Warn("Cannot send message to topic %q. MessagingService does not work in studio. Ignoring call.", self.Topic)
	end
end)
...
function CrossServerTopic:Subscribe()
	if not IS_ONLINE and WARN_WHEN_USING_FUNCTIONS_IN_STUDIO then
		Debug.Warn("Cannot subscribe to topic %q. MessagingService does not work in studio. Ignoring call.", self.Topic)
	else
...
function CrossServerTopic:Unsubscribe()
	if not IS_ONLINE and WARN_WHEN_USING_FUNCTIONS_IN_STUDIO then
		Debug.Warn("Cannot unsubscribe from topic %q. MessagingService does not work in studio. Ignoring call.", self.Topic)
	else
